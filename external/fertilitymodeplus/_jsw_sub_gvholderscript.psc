...
GlobalVariable		Property	GVScaleMethod		Auto	; GV for scaling method.
...
; 2.09
GlobalVariable		Property	GVGameDaysPassed	Auto	; GV equiv to GetCurrentGameTime()
; 2.28
GlobalVariable		Property	GVGameHour			Auto	; GV equal to the current game time, 15.25 = 3:15PM
...
;2.28
Float	Property	LastUpdateTime			=	-1.0	Auto	; the time of the last mod update
Float	Property	NextUpdateDue			=	9.2		Auto	;/ set to the hour when the next update is due, but actual update will be .2 
																	(12 minutes) after that value		/;
...
function	SetNextUpdateTime()
	...
	float	fTime	=	LastUpdateTime
	float	now		=	GVGameHour.GetValue()
	if fTime < 0.0
		fTime = now
	endIf
	fTime = (fTime as int) as float
	fTime += (0.2 + PollingInterval)
	while fTime < now
		fTime += 1.0
	endwhile
	while fTime > 24.0
