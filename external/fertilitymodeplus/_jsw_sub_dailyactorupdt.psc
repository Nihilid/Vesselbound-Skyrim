...
;	debug.trace("FM+: defined couples female "+ howMany + " male " + ParentsMale.GetSize() + " message " + ConfirmMessage.GetSize())
	...
	if (howMany < 1) || (howMany != ParentsMale.GetSize()) || (howMany != ConfirmMessage.GetSize())
		return
	endIf
	...
	int index = 0
	while (index < howMany)
		if Storage.CouplesQuestions[index] == 2
			Storage.CouplesQuestions[index] = AskPlayerQuestion(ParentsFemale.GetAt(index), ParentsMale.GetAt(index), ConfirmMessage.GetAt(index))
...
;		if thisActor
			...
			if !thisActor || (Storage.ActorBlackList.Find(thisActor as form) != -1) || !thisActor.GetDisplayName() || thisActor.IsDead()
			; 2.28 remove check on BlacklistByName
			; || \
;				(Storage.BlackListByName.Find((thisActor as objectReference).GetDisplayName()) != -1))
				...
				where = thisActor.GetCurrentLocation()
				if where
...
;	locName = ""
	...
	while (mIndex)
        mIndex -= 1
        thisActor = Storage.TrackedFathers[mIndex] as actor
;		if thisActor
			if !thisActor || (Storage.ActorBlackList.Find(thisActor as form) != -1) || thisActor.IsDead() || \
				(!_allowCreatures && thisActor.GetRace().HasKeyword(ActorTypeCreature))
				; 2.28 remove check on BlacklistByName
;				(Storage.BlackListByName.Find((thisActor as objectReference).GetDisplayName()) != -1) || \
