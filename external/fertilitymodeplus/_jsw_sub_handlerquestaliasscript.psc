...
;/	2.28 deprecated		
		...
		if _newDay
			_lastDay = today
		endIf/;
;		_newDay = GVHolder.LastUpdateTime > GVHolder.NextUpdateDue
		; 2.29
;/		bool newDay = (GVHolder.LastUpdateTime as int > GVHolder.GVGameHour.GetValue() as int) || (GVHolder.GVGameHour.GetValue() < 0.75)
        ScheduledUpdater.UpdateStatusAll(true, newDay)/;
		; 2.54
        ScheduledUpdater.UpdateStatusAll(true, ((GVHolder.LastUpdateTime as int > GVHolder.GVGameHour.GetValue() as int) || (GVHolder.GVGameHour.GetValue() < 0.75)))
		if GVHolder.VerboseMode
			fTime = Utility.GetCurrentRealTime() - fTime
;/			int tenths = ((fTime - ((fTime as int) as float)) * 10.0) as int
			Debug.Notification("FM+ Update Time: " + fTime as int + "." + tenths +"s")/;
			; 2.54
			Debug.Notification("FM+ Update Time: " + (fTime as int) + "." + (((fTime - ((fTime as int) as float)) * 10.0) as int) +"s")
		endIf
    endIf
	...
	; 2.28
	GVHolder.LastUpdateTime = GVHolder.GVGameHour.GetValue()
	RegisterNextUpdate()
endEvent
...
function	RegisterNextUpdate()
	...
	float fTime = GVHolder.NextUpdateDue - GVHolder.GVGameHour.GetValue()
	if fTime < 0.0
		fTime += 24.0
	endIf
	if fTime < 0.0
...
{Update tracking status after showracemenu is closed}
	...
	if GVHolder.Enabled
		...
		; 2.29 why a full update?  just add hte player!
;		ScheduledUpdater.UpdateStatusAll(false, GVHolder.LastUpdateTime > GVHolder.NextUpdateDue)
	else
