...
{copies array entries from fromIndex to toIndex}
	...
	Storage.FemPromiscuity[toIndex]				=	Storage.FemPromiscuity[fromIndex]
...
{truncates TrackedActors and associated arrays to length specified}
	...
	Storage.FemPromiscuity			=	Utility.ResizeIntArray(Storage.FemPromiscuity, newLength)
...
function	VerifyRaceNames()
	...
	; 2.31 change all GetForm()to GEtFormEX() just in case
	Game.GetFormEX(0x00013740).SetName("Argonian")	; Argonian
	Game.GetFormEX(0x0008883a).SetName("Argonian")	; Argonian Vampire
	Game.GetFormEX(0x00013741).SetName("Breton")		; breton
	Game.GetFormEX(0x0008883c).SetName("Breton")		; breton vampire
	Game.GetFormEX(0x00097a3d).SetName("Breton")		; afflicted
	Game.GetFormEX(0x00013742).SetName("Dark Elf")	; dark elf
	Game.GetFormEX(0x0008883d).SetName("Dark Elf")	; dark elf vampire
	Game.GetFormEX(0x00013743).SetName("High Elf")	; high elf
	Game.GetFormEX(0x00088840).SetName("High Elf")	; high elf vampire
	Game.GetFormEX(0x00013744).SetName("Imperial")	; imperial
	Game.GetFormEX(0x00088844).SetName("Imperial")	; imperial vampire
	Game.GetFormEX(0x00013745).SetName("Khajiit")		; khajiit
	Game.GetFormEX(0x00088845).SetName("Khajiit")		; khajiit vampire
	Game.GetFormEX(0x00013746).SetName("Nord")		; nord
	Game.GetFormEX(0x0007eaf3).SetName("Nord")		; astrid - useless for our purposes since not flagged playable
	Game.GetFormEX(0x00088794).SetName("Nord")		; nord vampire
	Game.GetFormEX(0x00013747).SetName("Orc")			; orc - why doesn't Imperious change this to Orsimer??
	Game.GetFormEX(0x000a82b9).SetName("Orc")			; orc vampire
	Game.GetFormEX(0x00013748).SetName("Redguard")	; redguard
	Game.GetFormEX(0x00088846).SetName("Redguard")	; redguard vampire
	Game.GetFormEX(0x00013749).SetName("Wood Elf")	; wood elf
	Game.GetFormEX(0x00088884).SetName("Wood Elf")	; wood elf vampire
	; 2.31
	int theForm = Game.GetModByName("Dawnguard.esm")
	theForm = Math.LeftShift(theForm, 24)
	theForm = Math.LogicalOR(theForm, 0x377d)
	; 2.09
;	debug.trace("FM+: CompactArrays DG form ID: " + theForm)
	Game.GetFormEX(theForm).SetName("Snow Elf")	; snow elves, which DG calls high elves for some reason...
