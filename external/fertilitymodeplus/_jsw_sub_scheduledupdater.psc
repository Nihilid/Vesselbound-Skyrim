...
{update all current tracked actors}
	...
	if isRegularPoll
		...
		while (index > 0)
			index -= 1
			currentActor = Storage.RefractoryPeriod[index]
			if (currentActor > 0)
				Storage.RefractoryPeriod[index] = (currentActor - 1)
			endIf
		endWhile
		currentActor = 0
		...
		while (currentActor < index)
			trackedActor = (Storage.TrackedActors[currentActor] as Actor)
			if trackedActor
				UpdateStatusSingleFull(trackedActor, currentActor, now, (trackedActor == playerRef), pcSpouse)
				trackedActor = none
			endIf
			currentActor += 1
		endWhile
...
;	update: profiling 16-33ms with pseudo-random RNG
	...
	bool trimester2 = recycleBool && ((timeNow - lastConcept) as int > _trimesterDuration) && ((timeNow - lastConcept) as int < _trimesterDuration * 2)
	...
	if (_newday && !recycleBool && (!recycleFloat || (birthday > _recoveryDuration)))
		...
		while ((dayToday > _cycleDuration) && (recycleInt < 5))
			...
			recycleInt += 1
		endWhile
		; 1.59 decrement OvulationBlock if they're starting a new cycle
		if (dayToday == 1)
    ...
    if (((autoInsemNPC && !isPlayerChar) || (autoInsemPC && isPlayerChar)) && (_autoInseminateChance != 0))
		if FetchRandom() < (_autoInseminateChance * (Storage.FemPromiscuity[actorIndex] + trimester2 as int))
...
{when called, re-rolls 20 of the values stored in the random array}
	...
	while (iterations)
		replacements = Math.LogicalAnd(replacements, 0x000003FF)
