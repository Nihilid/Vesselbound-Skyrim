	...
	function	UpdatePregnancy(Actor akActor, int actorIndex, float now, int pregnantDay = 0)
		...
		if (pregnantDay >= 0)
			if akActor.is3DLoaded()
				akActor.AddSpell(w234aew01, false)
				akActor.AddSpell(w234aew02, false)
			endIf
...
{Apply baby spawn or removal checks for the PC and NPCs}
	...
	bool grewUp = false
	actor child = none
	if baby && timeToGrowUp && (Storage.LastFatherForm[actorIndex] != none) && (Storage.UniqueMothers.Length > 0) && \
	(Storage.UniqueMothers.Find(akActor as form) != -1)
		; 2.59
		child = Util.CheckDefinedChild(akActor, actorIndex,  FMChildManager)
;		debug.trace("FM+: found defined child " + child + " for mother " + akActor.GetDisplayName())
	endIf

	if child
		grewUp = true
		Storage.ChildrenSpawned = Utility.ResizeFormArray(Storage.ChildrenSpawned, Math.LogicalAnd((Storage.ChildrenSpawned.Length + 1), 0xFF), child.GetLeveledActorBase() as form)
		Storage.LastFatherForm[actorIndex] = none
		; 2.59
;		(akActor as objectReference).PlaceAtMe(child, 1, true, false)
		child.MoveTo(akActor)
		child.Enable()
		(child as _SUB_FM_DefinedChildActivateScript).ItsAliiiIIIiiive()
		recycleInt= ModEvent.Create("FMDefinedChildSpawned")
		ModEvent.PushForm(recycleInt, child as form)
		ModEvent.Send(recycleInt)
;		SendModEvent("FMDefinedChildSpawned")
;		debug.trace("FM+: tried to spawn child " + child)
    ...
    if !grewUp && ((akActor == PlayerRef) || (Storage.LastFatherForm[actorIndex] == playerRef as form) || (akActor == pcSpouse))
        ...
        if (baby && timeToGrowUp)
			...
			if GVHolder.TrainingEnabled
				child = none
			else
				child = ((FMChildManager.GetNthAlias(0) as referenceAlias) as _JSW_SUB_LittleBlackBookAlias).SpawnAdoptableChild(akActor, akActor)
			endIf
			if child
				grewUp = true
				Storage.LastFatherForm[actorIndex] = none
...
;			    kidName = FMAdoptScript.GenerateName("Name your " + kidName)
				kidname = ((FMChildManager.GetAlias(0) as referenceAlias) as _JSW_SUB_LittleBlackBookAlias).GenerateName("Name your " + kidName)
