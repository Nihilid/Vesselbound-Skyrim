...
; 2.32 defunct and shrunken to zero length
...
; 2.28
int[]		Property	FemPromiscuity			Auto	; modifier to the tracked actor's promiscuity
...
; 2.29
Alias[]		Property	LaborAliases			Auto	; 
...
;		----  end 2.18  ----
; 2.31
int			Property	myLoadIndex				Auto	hidden	; the current load index of this mod's plugin
;	2.51
int			Property	maxAdoptions			Auto	hidden	;	the max adoptions allowed in BYOHRelationshipAdoption quest
																;	expected values are 2 for vanilla, or 6 with HMA
; keep this list updated, idiot
; 0 - trimester duration,int	1 - ovulation day, int	2 - deprecated	3 - deprecated
...
int[]		Property	FMValues				Auto	; store frequently-used variables.  Most are deprecated.

bool	function	CheckPresence2()
{verifies that the storage script is running}

	if !GVAlias || !GVAlias.ImHere()
		GVAlias = (GVHolder as quest).GetAlias(1) as _JSW_SUB_GVAlias
	endIf
	; 2.31
	myLoadIndex = Math.LogicalAND(0xFFFFF000, (GVHolder as quest).GetFormID())
	return true
...
{Dynamic update for storage in the latest version}
	...
	if FMValues.Length != 24
		FMValues = Utility.ResizeIntArray(FMValues, 24)
	endIf
	UpdateRequired = ((FMValues[10] != 61) || (FMValues[9] != 2))
;/	if ((FMValues[9] != 2) || (FMValues[10] != 32))
		FMValues[9] = 2
		FMValues[10] = 32
		UpdateRequired = true
	else
...
function	TrackedActorAdd(Actor akActor, location where = none, int flags = -2, location parentLoc = none)
		...
		elseIf akActor.HasFamilyRelationship()
			...
			while (theCount < FLLength)
				if akActor.HasAssociation(ExcludeAT.GetAt(theCount) as AssociationType)
					flags = Math.LogicalOR(flags, Math.LeftShift(0x02, theCount))
				endIf
				theCount += 1
			endWhile
		else
...
;	EventLock				=	Utility.ResizeIntArray(EventLock, anInt)
	...
	FemAttached				=	Utility.ResizeIntArray(FemAttached, anInt, -1)
	...
	; 2.28
	FemPromiscuity			=	Utility.ResizeIntArray(FemPromiscuity, anInt, 1)
...
function	TrackedFatherAdd(Actor akActor, location where = none, int flags = -2, location parentLoc = none)
...
;	MaleHasLoveInterest		=	Utility.ResizeFormArray(MaleHasLoveInterest, TFLength)
	MaleAttached			=	Utility.ResizeIntArray(MaleAttached, TFLength, -1)
...
{Removes all entries and shrinks arrays to 0 length}
	...
	; 2.26  whoops, removing them from factions should've been done sooner
...
;    EventLock				=	Utility.ResizeIntArray(EventLock, 0)
	...
	; 2.28
	FemPromiscuity			=	Utility.ResizeIntArray(FemPromiscuity, 0)
...
{block NPCs by name- will block ALL with the same display name}
	...
	if !actorName; 2.28 remove already-blocked names, too || (BlacklistByName.Find(actorName) != -1)
		; The specified actor has no name
		return
	endIf
	int multiPurposeIndex	=	BlackListByName.Find("")
	; 2.28
	if (BlacklistByName.Find(actorName) != -1)
		; they're already blacklisted, so don't add them again or extend the array
	elseIf (multiPurposeIndex == -1)
...
; 2.15
	if forMCM || (anInt == 0) || ((anInt == 1) && (CurrentFatherRace[actorIndex] == none)) || ((anInt == 2) && \
		...
		raceName = (akActor.GetRace() as form).GetName()
	else
	; 2.15
...
endFunction
; 2.30 Get for misc uses
int	function	GetActorIndex(actor akActor)

	if akActor && (TrackedActors.Length > 0)
		return TrackedActors.Find(akActor as form)
	endIf
	return -1
endFunction
