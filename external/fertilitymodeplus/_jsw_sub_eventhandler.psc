...
{applies labor spell to target actor}
	...
	while (!ready && (index > 0))
		index -= 1
		ready = !(Storage.LaborAliases[index] as referenceAlias).GetReference() && !Storage.LaborAliases[index].GetOwningQuest().IsRunning()
	endWhile
	if !ready
		return
	endIf
	(Storage.LaborAliases[index] as referenceAlias).ForceRefTo(targetActor as objectReference)
	Storage.LaborAliases[index].GetOwningQuest().SetCurrentStageID(10)
	if Storage.LaborAliases[index].GetOwningQuest().IsRunning()
		if GVHolder.VerboseMode
...
;	Actor kActor = akTarget as Actor
	int recycleInt = Util.GetActorGender(akTarget as Actor)
	; 1.43 fix bad logic
	if (recycleInt != 1)
		return	; target is not tracked
	else
		Util.AddToTracking(akTarget as Actor, recycleInt)
    ...
    if (recycleInt != -1)	; check if actor is female and tracked
		...
		if (akTarget as Actor == PlayerRef)
			ModEvent.Send(ModEvent.Create("FMPUpdateWidgetContent"))
		endIf
    endIf
endEvent
