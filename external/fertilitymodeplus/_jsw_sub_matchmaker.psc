...
{if an actor has a spouse/courting association try to locate them}
	...
	actorIndex = herIndex
	if Storage.Fidelity[actorIndex] != 0
		bFaithful = ((spouseInsemChance + Storage.Fidelity[actorIndex]) > faithRoll)
	else; arbitrarily assume the male has a RelationshipRank of 3
		bFaithful = ((spouseInsemChance + 7) > faithRoll)
	endIf
	if bFaithful && (Storage.FemAttached[actorIndex] > 0)
...
{returns true if they're blood relatives closer than first cousins}
	...
	while !related && (index > 0)
		index -= 1
		related = (Math.LogicalAND(combinedFlags, Math.LeftShift(0x02, index)) as bool) && firstActor.HasAssociation(ExcludeAT.GetAt(index) as AssociationType, secondActor)
	endWhile
	return related
